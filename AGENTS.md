...
> and its flagship product Orasaka. This file is the single source of truth for
> how the site is built, themed, and kept in sync with the Orasaka engine.
> The product engine has its **own** contract at `products/orasaka/AGENTS.md` —
> do not mix the two.
...
- A **Next.js (App Router)** static-first site: marketing, documentation, and
  **code-driven interactive visualizations** of the Orasaka architecture.
- It is the **read side**. Architecture/doc facts come from the Orasaka source —
  the site never hand-authors them.

## 2. Content & data are generated, not hand-written

- `orasaka-content/docs/` and `app/data/architecture.json` are **synced from
  Orasaka** via `orasaka docs sync` (run from `products/orasaka`). **Never
  hand-edit** files under `orasaka-content/` or `app/data/` — change the Orasaka
  source and re-sync.
...
*Agent-neutral contract for `krizaka-com`. The Orasaka engine contract lives at
`products/orasaka/AGENTS.md`.*
