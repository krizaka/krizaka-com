...
- **Status**: Accepted
- **Date**: 2026-06-10
- **Scope**: `products/orasaka` (router app + framework modules)
- **Supersedes**: the scattered `orasaka.*` domain-config keys previously held in `application.yml`
...
as `orasaka.features.*` yaml **and** in two separate DB tables/consumers
(`orasaka_feature_flags`, `FeatureRegistryProperties`); per-interceptor `enabled`
flags existed in yaml **and** in `pipeline_interceptor_config`; the same `UPLOAD_DIR`
was bound under two different keys; several `orasaka.*` values were read via loose
...
`@ConfigurationProperties` record — no loose `@Value` for `orasaka.*`. All domain data
and behaviour is read from the database as the single source of truth:

| Domain concern | Single source (DB) | Read through |
|:---|:---|:---|
| Operation-graph capabilities (blueprint + enabled) | `orasaka_capabilities` | `CapabilityProvider` port → `GraphEngine`, `AdminFeatureController`, `BootstrapController` |
...
| Default model per category | `orasaka_models.is_default` | catalog model service (no yaml fallback) |
| Rate-limit default tier | `orasaka_rate_limits.is_default` | `RateLimitProvider` |

`application.yml` retains only: master switches (`orasaka.core.orchestration.enabled`,
`orasaka.security.disable-ai`), routing mode, model-wiring for the refiner/router AI
stages, and pure infra (`orasaka.router.uploads`, `orasaka.jobs`, `orasaka.messaging`,
`orasaka.identity.crypto`, datasource/broker, OAuth client wiring).

### Migration phases (commit-per-green)

- **Phase 2** — media default video model resolves from `orasaka_models.is_default`; dropped `orasaka.media.video-default-model`.
- **Phase 3** — rate-limit default tier resolves from `orasaka_rate_limits.is_default`; deleted `RateLimitProperties`.
- **Phase 4** — capability registry → `orasaka_capabilities` (supersedes `orasaka_feature_flags` + `orasaka.features.*` yaml + `FeatureRegistryProperties`); new core `CapabilityProvider` port + `CapabilityDescriptor`.
- **Phase 5** — per-interceptor enable → `pipeline_interceptor_config`; dropped `orasaka.core.orchestration.{user-context,system-context,refiner,router}.enabled`.
...
models, and tiers at runtime (no redeploy); no yaml/DB drift; every `orasaka.*` is a
