 ...
 *
 * Source of truth = the Orazaka backend; regenerate via `orazaka docs build`.
 */
...
function inferLayer(id: string): Layer {
  ...
  if (id.includes("router") || id.includes("worker") || id.includes("client") || id.includes("cli") || id.includes("admin")) return "app";
  return "framework";
}
...
function useLayout() {
  return useMemo(() => {
    ...
    const getBand = (m: Mod): number => {
      const id = m.id;
      if (id.includes("persistence") || id.includes("tools")) return 0; // Foundation
      if (id === "orazaka-core" || id === "orazaka-identity" || id === "orazaka-interceptors") return 1; // Core
      if (id === "orazaka-business") return 2; // Capabilities
      if (id === "orazaka-router" || id.includes("worker") || id === "orazaka-cli") return 3; // Application
      if (id.includes("client") || id.includes("web-") || id.includes("admin")) return 4; // Aggregate / UI
      return 0;
    };

    const band: Record<string, number> = {};
    mods.forEach((m) => (band[m.id] = getBand(m)));
